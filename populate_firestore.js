...
async function clearCollection() {
  ...
  const snapshot = await firestore.collection("qa_pairs").limit(1000).get();
...
async function populateFirestore() {
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      const qa = data[i];
      const docRef = firestore.collection("qa_pairs").doc();
      ...
      if (batchCount === BATCH_SIZE || i === data.length - 1) {
        ...
        const rate = ((totalAdded / (Date.now() - startTime)) * 1000).toFixed(
          0
        );
        console.log(
          `✓ Progress: ${totalAdded}/${data.length} (${(
            (totalAdded / data.length) *
            100
          ).toFixed(1)}%) | ${elapsed}s elapsed | ${rate} docs/sec`
        );
    ...
    const snapshot = await firestore.collection("qa_pairs").count().get();
