...
function initializeFirestore() {
  if (config.isLocal) {
    ...
    firestore = new Firestore({
      ...
      host: config.firestoreEmulatorHost,
      ssl: false,
    });
  } else {
...
async function loadQACache() {
  try {
    ...
    const snapshot = await firestore.collection(config.qaCollection).get();
...
async function findAnswerByIntent(intentName) {
  try {
    ...
    const match = qaCache.find(
      (qa) => qa.intent && qa.intent.toLowerCase() === intentName.toLowerCase()
    );
...
async function addQAPair(intent, question, answer) {
  try {
    const docRef = await firestore.collection(config.qaCollection).add({
